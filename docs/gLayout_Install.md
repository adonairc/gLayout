# A6lib DEV Environment Setup 

> 🚧 **Under Construction** 

Instructions for A6lib specific steps. For A6lib, we are going to build the Container with additional steps (with the same scripts) to forward Port: 8888 (alongside VNC or X11) to start a Jupyter Server to execute A6lib Notebooks.  

\* Note: A6lib repo can be cloned or downloaded from [here](https://github.com/ReaLLMASIC/gLayout/tree/main). You will need it for Tutorials.
...
This will set up and install the necessary components (will take some time) for the *A6lib* and will start a Jupyter server to access it. You can access it from your browser [http://localhost:8888/lab]() or VS-Code.
...
Then you should see the GLdev Kernel. Don’t forget to choose it for all A6lib code.

![](_images/gs/Nimage8.png)

### Installing KLive and PDK Colour Schemes

To visualise the GDS files generated by the A6lib, you can use the `_.show()` function of the *gdsfactory*. For this to work, you need to have *Klive* plugin installed and running in the *Klayout*. 
...
You can see all active dockers with `docker ps -a`.Replace `a6libcontainer` with your container name or ID. 

1. To remove the container, run the command:   
   1. (Mac) `docker container rm a6libcontainer`  
   2. (Linux/WSL) `docker rm a6libcontainer`  
2. To stop a container, run the command:  
   1. (Mac) `docker container stop a6libcontainer`  
   2. (Linux/WSL) `docker stop a6libcontainer`  
3. To restart the container, run the command:   
   1. (Mac) `docker container restart a6libcontainer`  
   2. (Linux/WSL) `docker restart a6libcontainer`  
4. To execute a running container, first check its status by running   
   1. (Mac) `docker container ls -a` (Linux/WSL) `docker ls -a`  
   2. Then `docker exec -it a6libcontainer /bin/bash` (if *a6libcontainer* is running) (`-it` flag runs the container in interactive mode)   
