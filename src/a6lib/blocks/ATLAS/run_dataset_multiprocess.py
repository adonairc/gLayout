...
# -----------------------------------------------------------------------------
# Ensure the *local* `a6lib` package is discoverable *before* we import any
# module that depends on it (e.g. `robust_verification`).
# -----------------------------------------------------------------------------
_here = Path(__file__).resolve()
_a6lib_repo_path = _here.parent.parent.parent.parent.parent.parent
...
if not _a6lib_repo_path.exists():
    _a6lib_repo_path = pwd_path / "../../../../"

if _a6lib_repo_path.exists() and str(_a6lib_repo_path) not in sys.path:
    sys.path.insert(0, str(_a6lib_repo_path))

del _here, _a6lib_repo_path
...
def get_global_pdk():
    ...
    if GLOBAL_SKY130_PDK is None:
        from a6lib.pdk.sky130_mapped import sky130_mapped_pdk as _pdk
...
def setup_environment():
    ...
    # Ensure the `a6lib` package directory is discoverable regardless of
    # how the user launches the script.
    a6lib_path = pwd_path / "../../../../"
    print("Using a6lib path:", a6lib_path)
    if a6lib_path not in sys.path:
        sys.path.insert(0, a6lib_path)

    # Prepend to PYTHONPATH so subprocesses (if any) inherit the correct path
    current_pythonpath = os.environ.get('PYTHONPATH', '')
    if a6lib_path not in current_pythonpath.split(":"):
        os.environ['PYTHONPATH'] = f"{a6lib_path}:{current_pythonpath}"
...
def robust_transmission_gate(_, **params):
    ...
    ``a6lib.pdk.sky130_mapped`` each time and passing that brand-new
...
def run_single_evaluation(trial_num, params, output_dir):
    ...
    try:
        with chdir(trial_work_dir):
            ...
            if 'a6lib.pdk.sky130_mapped' in sys.modules:
                importlib.reload(sys.modules['a6lib.pdk.sky130_mapped'])
