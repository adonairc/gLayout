...
class PortTree:
	"""PortTree helps a a6lib.programmer visualize the ports in a component
	...
	def print(self, savetofile: bool=True, default_opts: bool=True, depth: int | None = None, outfile_name: str | None = None, **kwargs):
		...
		default_opts = bool=True results in using a6lib.recommended default print arguments
...
def print_port_tree_all_cells() -> list:
	"""print the PortTree for most of the a6lib.cells and save as a text file.
	returns a list of components
	"""
	from a6lib.primitives.via_gen import via_stack, via_array
	from a6lib.opamp import opamp
	from a6lib.primitives.mimcap import mimcap
	from a6lib.primitives.mimcap import mimcap_array
	from a6lib.primitives.guardring import tapring
	from a6lib.primitives.fet import multiplier, nmos, pmos
	from a6lib.diff_pair import diff_pair
	...
	for name, py_cell in celllist:
		from a6lib import __version__ as a6libinfo
		va6lib = str(a6libinfo)
		PortTree(py_cell,name=name).print(depth=5,outfile_name=name+"_v"+va6lib+"_tree.txt",default_orientation=True)
	return celllist
