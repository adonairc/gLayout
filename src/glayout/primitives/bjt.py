...
def fill_area_with_contacts(pdk: MappedPDK, dims, center, contact_layer:str,
    ...
    component = Component(name=f"contacts_{contact_layer}")
...
def draw_metal_over_ring(pdk: MappedPDK,ring_dims:dict[str,np.ndarray[Any]],
    ...
    component = Component(name=f"metal_ring_{metal_layer}")
...
def fill_ring_with_contacts(pdk: MappedPDK,ring_dims:dict[str,np.ndarray[Any]],
    ...
    #print(l_metal_dims)
    component = Component(name=f"ring_contacts_{contact_layer}")
...
def draw_bjt(pdk: MappedPDK, active_area: tuple[float,float], bjt_type: str,
    ...
    component = Component(name=f"bjt_{bjt_type}_{active_area[0]}x{active_area[1]}")
...
def __mult_array_macro(
    ...
    # recenter
    final_arr = Component(name=f"bjt_mult_array_{bjt_type}_m{multipliers}")
...
def pnp(
    ...
    ****using the tuple option, you can specify a single side dummy such as true,false
    ...
    """
    pdk.activate()
    pnp = Component(name=f"pnp_{active_area[0]}x{active_area[1]}_m{multipliers}")
    if rmult:
        if rmult<1:
            raise ValueError("rmult must be positive int")
        bc_rmult = rmult
...
def npn(
    ...
    ****using the tuple option, you can specify a single side dummy such as true,false
    ...
    """
    pdk.activate()
    npn = Component(name=f"npn_{active_area[0]}x{active_area[1]}_m{multipliers}")
    if rmult:
        if rmult<1:
            raise ValueError("rmult must be positive int")
        bc_rmult = rmult
